...
int main(int argc,char **argv)
{
  ...
  ierr = PetscOptionsGetInt(PETSC_NULLPTR,PETSC_NULLPTR,"-N",&N,PETSC_NULLPTR);CHKERRQ(ierr);
  ...
  // ierr = MatSetSizes(J,PETSC_DECIDE,PETSC_DECIDE,N+1,N+1); CHKERRQ(ierr);
...
int jacfun(SNES snes,Vec x,Mat jac,Mat jac1,void *data) {
  ...
  for (j=1; j<ctx.N; j++) {
    ...
    ierr = MatSetValues(jac,1,&j,1,&j,&A,ADD_VALUES); 
